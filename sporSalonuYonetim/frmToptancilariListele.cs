...
{
    public partial class frmToptancilariListele : Form
    {
        public static GirisBilgi User { get; set; }
        int toptanciid;
        sqlbaglanti gnl = new sqlbaglanti();
        SqlDataAdapter da;
        public frmToptancilariListele()
        {
            InitializeComponent();
            #region yetkiler
        ...
        private void toptancisil(int toptanciid)
        {
            SqlConnection con = new SqlConnection(gnl.conString);
            con.Open();
            string sql = "DELETE FROM Toptanci where toptanciID=@id";
            SqlCommand cmd = new SqlCommand(sql, con);
            cmd.Parameters.AddWithValue("@id", toptanciid);
            cmd.ExecuteNonQuery();
            con.Close();
        }
        private void toptancilistele()
        {
            SqlConnection con = new SqlConnection(gnl.conString);
            con.Open();
            da = new SqlDataAdapter("SELECT toptanciID,toptanciAdi, toptanciTelefon,toptanciAdres FROM Toptanci", con);
            DataTable tablo2 = new DataTable();
            da.Fill(tablo2);
            dataGridView1.DataSource = tablo2;
            con.Close();
        }
        private void toptancissayisi()
        {
            SqlConnection con = new SqlConnection(gnl.conString);
            string sorgu = "SELECT COUNT(toptanciID) FROM Toptanci";
            int durum;
        ...
        private void frmToptancilariListele_Load(object sender, EventArgs e)
        {
            toptancilistele();
            toptancissayisi();
        }
        ...
        private void dataGridView1_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)//farenin sağ tuşuna basılmışsa
            {
                ...
                if (satir > -1)
                {
                    ...
                    toptanciid = Convert.ToInt32(dataGridView1.Rows[satir].Cells["toptanciID"].Value);
                }
            }
        }
