...
{
    public partial class frmUrunSatisEkrani : Form
    {
        ...
        public frmUrunSatisEkrani()
        {
            ...
            #endregion
            DataGridSettings(dataAbone);
        }

        public void DataGridSettings(DataGridView datagridview)
        {
            dataAbone.RowHeadersVisible = false;
        }
        #region menu
        ...
        private void frmUrunSatisEkrani_Load(object sender, EventArgs e)
        {
            // TODO: Bu kod satırı 'salonYonetimDataSet16.KayitTable' tablosuna veri yükler. Bunu gerektiği şekilde taşıyabilir, veya kaldırabilirsiniz.
            this.kayitTableTableAdapter.Fill(this.salonYonetimDataSet16.KayitTable);

        }

        private void dataAbone_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            frmUrunSatis frmUrunSatis = new frmUrunSatis();
            frmUrunSatis.adi = dataAbone.CurrentRow.Cells[0].Value.ToString();
            frmUrunSatis.tckimlikno = dataAbone.CurrentRow.Cells[1].Value.ToString();
            frmUrunSatis.telefon = dataAbone.CurrentRow.Cells[2].Value.ToString();
            frmUrunSatis.Show();
        }
        ...
        private void dataAbone_MouseClick(object sender, MouseEventArgs e)
        {
            
        }

        private void dataAbone_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                this.dataAbone.Rows[e.RowIndex].Selected = true;
                this.rowIndex = e.RowIndex;
                this.dataAbone.CurrentCell = this.dataAbone.Rows[e.RowIndex].Cells[1];
                this.contextMenuStrip1.Show(this.dataAbone, e.Location);
        ...
        {
            if (!this.dataAbone.Rows[this.rowIndex].IsNewRow)
            {
                this.dataAbone.Rows.RemoveAt(this.rowIndex);
            }
        }
    }
}
