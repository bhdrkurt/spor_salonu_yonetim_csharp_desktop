...
{
    public partial class frmUrunKategorileri : Form
    {
        public static GirisBilgi User { get; set; }
        public frmUrunKategorileri()
        {
            InitializeComponent();
        ...
        private void btnPaketKayit_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            SqlCommand selectsorgu = new SqlCommand("SELECT Kategoriler.kategoriAdi FROM Kategoriler WHERE kategoriAdi='" + txtKategoriAdi.Text + "'", con);
            ...
            if (kayitkontrol == false)
            {
                ...
                {
                    label2.Visible = true;
                    label2.Text = "Lütfen Kategori İsmi giriniz.";
                }
                ...
                if(txtKategoriAdi.Text != "")
                {
                    ...
                    {
                        con.Open();
                        SqlCommand komut = new SqlCommand("INSERT INTO Kategoriler(kategoriAdi) VALUES (@p1)", con);
                        komut.Parameters.AddWithValue("@p1", txtKategoriAdi.Text);
                        komut.ExecuteNonQuery();
                        con.Close();
                        label2.ForeColor = Color.Green;
                        label2.Text = "Kayıt Tamamlanmıştır.";
                        txtKategoriAdi.Text = "";
                        label5.Text = "";
                    }
                    catch (Exception)
